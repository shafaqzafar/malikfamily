...
export type Shift = { id: string; name: string; start?: string; end?: string; graceMin?: number; absentCharges?: number; lateDeduction?: number; earlyOutDeduction?: number }
...
export default function Pharmacy_StaffReportDialog({ open, onClose, staffList, initialMonth, initialStaffId }: Props){
    ...
    ;(async()=>{
      try {
        const [shiftRes, settingsRes] = await Promise.all([
          pharmacyApi.listShifts(),
          pharmacyApi.getSettings().catch(()=>null),
        ])
        if (!mounted) return
        setShifts((shiftRes.items||[]).map((x:any)=>({ id: x._id, name: x.name, start: x.start, end: x.end, graceMin: x.graceMin, absentCharges: x.absentCharges, lateDeduction: x.lateDeduction, earlyOutDeduction: x.earlyOutDeduction })))
  ...
  // Sync incoming props when dialog opens
  useEffect(()=>{
    if (!open) return
    if (initialStaffId && initialStaffId !== selectedId) setSelectedId(initialStaffId)
    ...
    ;(async()=>{
      try {
        ...
        const [res, earnRes] = await Promise.all([
          pharmacyApi.listAttendance({ from, to, staffId: selected.id }),
  ...
  const stats = useMemo(()=>{
    ...
    for (const r of daily){
      ...
      if (!sh) continue
      const grace = Number(sh.graceMin || 0)
    ...
    // Derived absents = missing days (workingDays - present - leave)
  ...
  const exportCsv = () => {
    ...
    for (const r of daily){ rows.push([r.date, shiftName(r.shift), r.status, r.clockIn||'', r.clockOut||'', String(r.minutes)]) }
