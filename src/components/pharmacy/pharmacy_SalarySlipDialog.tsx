...
export default function Pharmacy_SalarySlipDialog({ open, onClose, expense, staffName, month, basicSalary, netSalary, paidToDate }: Props){
  const [info, setInfo] = useState<{ name: string; phone: string; address: string; footer: string; logo: string }>({ name: 'PHARMACY', phone: '', address: '', footer: '', logo: '' })
  ...
  return (
    <>
      <style>{`
        ...
        #pharmacy-salary-slip { font-family: 'Poppins', Arial, sans-serif }
        .tabular-nums { font-variant-numeric: tabular-nums }
        @media print {
          @page { size: 80mm auto; margin: 0 }
          body { -webkit-print-color-adjust: exact; print-color-adjust: exact }
          body * { visibility: hidden !important }
          #pharmacy-salary-slip, #pharmacy-salary-slip * { visibility: visible !important }
          #pharmacy-salary-slip { position: absolute !important; left: 0; right: 0; top: 0; margin: 0 auto !important; padding: 0 6px !important; width: 440px !important; box-sizing: content-box !important; line-height: 1.25; overflow: visible !important; z-index: 2147483647 }
          .no-print { display: none !important }
          .only-print { display: block !important }
          #pharmacy-salary-slipe hr { border-color: #000 !important }
          #pharmacy-salary-slip, #pharmacy-salary-slip * { color: #000 !important }
        }
      `}</style>
      <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 px-4 py-6 print:static print:p-0 print:bg-white" role="dialog" aria-modal="true">
        <div className="w-full max-w-2xl overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black/5 print:shadow-none print:ring-0 print:rounded-none print:max-w-none print:bg-white">
          ...
          <div className="max-h-[75vh] overflow-y-auto px-6 py-6 print:p-0 print:overflow-visible">
            <div id="pharmacy-salary-slip" className="mx-auto w-[440px] print:w-[440px]">
              ...
              {expense?.note ? (
                <div className="mt-3 border-t border-dashed pt-2 text-xs text-slate-700 break-words print:text-black">{expense.note}</div>
              ) : null}
