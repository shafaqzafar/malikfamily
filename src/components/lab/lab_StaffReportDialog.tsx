...
import Lab_SalarySlipDialog from './lab_SalarySlipDialog'
...
export default function Lab_StaffReportDialog({ open, onClose, staffList, initialMonth, initialStaffId }: Props){
    ...
    ;(async()=>{
      try {
        ...
        const [res, earnRes] = await Promise.all([
          labApi.listAttendance({ from, to, staffId: selected.id }),
          labApi.listStaffEarnings({ from, to, staffId: selected.id, limit: 1000 }),
        ])
        if (!mounted) return
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      try {
        ...
        const dt = new Date(Number(month.slice(0,4)), Number(month.slice(5,7)), 0)
        const to = `${dt.getFullYear()}-${String(dt.getMonth()+1).padStart(2,'0')}-${String(dt.getDate()).padStart(2,'0')}`
        const res: any = await labApi.listExpenses({ from, to, type: 'Salaries', search: selected.name, limit: 1000 as any })
        if (!mounted) return
  ...
  const createSalaryExpense = async (amount: number, mode: 'full'|'half'|'custom', extraNote?: string) => {
    ...
    try {
      ...
      const created: any = await labApi.createExpense({
        date: today,
        type: 'Salaries',
      ...
      setSlipOpen(true)
      try { window.dispatchEvent(new Event('lab:expenses:refresh')) } catch {}
    } catch {
  ...
  const exportPdf = async () => {
    ...
    doc.setFontSize(14)
    doc.text(String(settings?.labName || 'Lab'), margin, margin + 4)
    doc.setFontSize(9)
    let topY = margin + 10
    ...
    doc.save(`lab-staff-report-${staffName}-${month}.pdf`)
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 px-4 py-8">
      ...
      <Lab_SalarySlipDialog open={slipOpen} onClose={()=>setSlipOpen(false)} expense={slipExpense} staffName={selected?.name} month={month} basicSalary={basicSalary} netSalary={netSalary} paidToDate={slipPaidToDate} />
    </div>
  )
}
