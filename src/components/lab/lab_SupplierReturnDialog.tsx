...
type PurchaseLine = { itemId?: string; name: string; totalItems: number; unitsPerPack?: number; buyPerUnitAfterTax?: number; buyPerUnit?: number; buyPerPack?: number; }
...
export default function Lab_SupplierReturnDialog({ open, onClose, purchase, onSubmitted }: Props){
  const [qtys, setQtys] = useState<Record<number,string>>({})
  const [note, setNote] = useState('')

  const calc = useMemo(() => {
    if (!purchase) return { lines: [] as any[], total: 0 }
    const lines = purchase.lines.map((l, idx) => {
      ...
      const amt = unit * rq
      return { idx, name: l.name, unitPrice: unit, purchasedQty: Number(l.totalItems || 0), returnQty: rq, amount: Math.round(amt*100)/100, itemId: l.itemId }
    })
  ...
  async function submit(){
    ...
    const retLines = calc.lines.filter(x => x.returnQty>0).map(x => {
      ...
      if (x.itemId) base.itemId = x.itemId
      return base
    })
    if (!retLines.length){ onClose(); return }
    const body = { type: 'Supplier' as const, datetime: new Date().toISOString(), reference: purchase.invoice, party: purchase.supplierName || '', note: note.trim() || undefined, lines: retLines }
    try {
      const res = await labApi.createReturn(body)
      const retDoc = (res as any)?.return || res
      onSubmitted({ purchase: (res as any)?.purchase || null, returnDoc: retDoc })
      try { window.dispatchEvent(new CustomEvent('lab:return', { detail: { reference: purchase.invoice } })) } catch {}
    } catch (err: any) {
      ...
      <div className="w-full max-w-3xl overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black/5">
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <input value={note} onChange={e=>setNote(e.target.value)} placeholder="Reason (optional)" className="w-56 rounded-md border border-slate-300 px-2 py-1 text-sm" />
