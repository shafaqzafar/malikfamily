...
export default function Hospital_CashCountSlipDialog({ open, onClose, entry }: Props) {
  const [info, setInfo] = useState<{ name: string; phone: string; address: string; footer: string; logo: string }>({ name: 'HOSPITAL', phone: '', address: '', footer: 'Have a nice day!', logo: '' })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!mounted) return
        setInfo({
          name: s.hospitalName || s.name || 'HOSPITAL',
          phone: s.phone || '',
          address: s.address || '',
