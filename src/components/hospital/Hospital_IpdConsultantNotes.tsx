...
export default function ConsultantNotes({ encounterId }: { encounterId: string }){
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4" data-encounterid={encounterId}>
      ...
      <ConsultantDialog open={open} onClose={()=>setOpen(false)} onSave={add} />
    </div>
  )
}

function ConsultantDialog({ open, onClose, onSave }: { open: boolean; onClose: ()=>void; onSave: (d: { when?: string; text?: string; doctorName?: string; sign?: string })=>void }){
  if(!open) return null
        ...
        <div className="px-5 py-4 text-sm">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              ...
              <input id="cn-when" name="when" type="datetime-local" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
            <div>
              <label htmlFor="cn-doctor" className="block text-xs font-medium text-slate-600">Doctor Name</label>
              <input id="cn-doctor" name="doctorName" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
            <div>
              <label htmlFor="cn-sign" className="block text-xs font-medium text-slate-600">Sign</label>
              <input id="cn-sign" name="sign" className="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
