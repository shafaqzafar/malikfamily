...
export default function Hospital_StaffEarningsDialog({ open, onClose, staff, month }: { open: boolean; onClose: ()=>void; staff: { id: string; name: string }; month?: string }){
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      setLoading(true)
      try{
        const res = await hospitalApi.listStaffEarnings({ staffId: staff.id, from: firstDay, to: lastDay, limit: 100 })
        if (!mounted) return
        setItems((res.items||[]))
  ...
  const save = async () => {
    ...
    if (editId){
      await hospitalApi.updateStaffEarning(editId, payload)
    } else {
      await hospitalApi.createStaffEarning(payload)
    }
    resetForm()
    setEditId(null)
    // reload
  ...
  const onDelete = async (id?: string) => {
    if (!id) return
    await hospitalApi.deleteStaffEarning(id)
    const res = await hospitalApi.listStaffEarnings({ staffId: staff.id, from: firstDay, to: lastDay, limit: 100 })
    setItems((res.items||[]))
  }
