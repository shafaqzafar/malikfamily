...
import type { Supplier } from './aesthetic_AddSupplierDialog'
...
export default function Pharmacy_SupplierDetailsDialog({ open, onClose, supplier }: Props) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        let arr1: any[] = []
        let arr2: any[] = []
        try { const r1 = await aestheticApi.listSupplierPurchases(supplier.id); arr1 = r1.items || [] } catch {}
        if (supplier.name){ try { const r2 = await aestheticApi.listPurchases({ search: supplier.name, limit: 500 }); arr2 = r2.items || [] } catch {} }
        if (!mounted) return
