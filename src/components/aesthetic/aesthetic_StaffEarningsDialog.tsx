...
export default function Aesthetic_StaffEarningsDialog({ open, onClose, staff, month }: { open: boolean; onClose: ()=>void; staff: { id: string; name: string }; month?: string }){
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      setLoading(true)
      try{
        const res = await aestheticApi.listStaffEarnings({ staffId: staff.id, from: firstDay, to: lastDay, limit: 100 })
        if (!mounted) return
        setItems((res.items||[]))
  ...
  const save = async () => {
    ...
    if (editId){
      await aestheticApi.updateStaffEarning(editId, payload)
    } else {
      await aestheticApi.createStaffEarning(payload)
    }
    resetForm()
    setEditId(null)
    // reload
  ...
  const onDelete = async (id?: string) => {
    if (!id) return
    await aestheticApi.deleteStaffEarning(id)
    const res = await aestheticApi.listStaffEarnings({ staffId: staff.id, from: firstDay, to: lastDay, limit: 100 })
    setItems((res.items||[]))
  }
