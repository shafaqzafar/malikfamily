...
export default function Aesthetic_TokenSlip({ open, onClose, data, autoPrint = false, user }: { open: boolean; onClose: ()=>void; data: TokenSlipData; autoPrint?: boolean; user?: string }){
  const [settings, setSettings] = useState({ name: 'Aesthetic Clinic', phone: '', address: '', logoDataUrl: '', slipFooter: 'Powered by Hospital MIS' })
  ...
  useEffect(() => {
    ...
    async function load(){
      try {
        if (!settingsCache) settingsCache = await aestheticApi.getSettings()
        if (!cancelled && settingsCache) {
          const s: any = settingsCache
          setSettings({
            name: s.pharmacyName || s.name || 'Aesthetic Clinic',
            phone: s.phone || '',
            address: s.address || '',
            logoDataUrl: s.logoDataUrl || '',
            slipFooter: s.billingFooter || s.slipFooter || 'Powered by Hospital MIS',
          })
        }
      } catch {}
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 print:bg-white print:static">
      <div id="hospital-receipt" className="w-[360px] rounded-md border border-slate-300 bg-white p-4 shadow print:shadow-none print:border-0 print:w-[300px]">
        ...
        {data.procedurePrice != null ? (
          <div className="space-y-1 text-sm text-slate-800">
            <Row label="Procedure Cost:" value={Number(data.procedurePrice||0).toFixed(2)} />
            <Row label="Discount:" value={Number(data.procedureDiscount||0).toFixed(2)} />
            <Row label="Paid Today:" value={Number(data.procedurePaidToday||0).toFixed(2)} />
            <Row label="Paid To Date:" value={Number(data.procedurePaidToDate||0).toFixed(2)} />
            <Row label="Remaining Balance:" value={Number(data.procedureBalanceAfter||0).toFixed(2)} boldValue />
          </div>
        ) : (
          <div className="space-y-1 text-sm text-slate-800">
