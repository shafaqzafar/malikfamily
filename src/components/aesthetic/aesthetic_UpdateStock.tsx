...
export default function Aesthetic_UpdateStock({ open, onClose }: Props) {
  ...
  useEffect(()=>{
    ...
    aestheticApi.listInventory({ limit: 1000 }).then((res:any)=>{
      if (!mounted) return
  ...
  // Prefill from previous inventory when a medicine is selected
  useEffect(()=>{
    if (!open) return
    const key = (name||'').trim()
    if (!key) return
    let mounted = true
    aestheticApi.listInventory({ search: key, limit: 50 }).then((res:any)=>{
      if (!mounted) return
      const items: any[] = res?.items || []
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          <div className="mb-4 grid gap-4 sm:grid-cols-3">
            <div>
              ...
              <input list="inventory-meds" value={name} onChange={e=>setName(e.target.value)} className="w-full rounded-md border border-amber-400 px-3 py-2 text-sm" placeholder="Medicine name" />
