...
import Lab_SupplierReturnDialog from '../../components/lab/lab_SupplierReturnDialog'
...
export default function Lab_SupplierReturns() {
  ...
  const [slip, setSlip] = useState<{ open:boolean; billNo:string; party?:string; lines:{ name:string; qty:number; amount:number }[]; total:number; note?: string }>({ open:false, billNo:'', party:'', lines:[], total:0, note: '' })

  useEffect(() => {
    let mounted = true
    ;(async () => {
      try {
        const res: any = await labApi.listPurchases({ from: from || undefined, to: to || undefined, search: (invoice || supplier) || undefined, page, limit })
    ...
    <div className="space-y-4">
      ...
      <Lab_SupplierReturnDialog
        ...
        purchase={selected ? {
          ...
          lines: (selected.lines || []).map((l:any)=>({ itemId: l.itemId, name: l.name, totalItems: l.totalItems, unitsPerPack: l.unitsPerPack, buyPerUnitAfterTax: l.buyPerUnitAfterTax, buyPerUnit: l.buyPerUnit, buyPerPack: l.buyPerPack }))
        } : null}
        onSubmitted={({ returnDoc })=>{
          ...
          setSlip({ open: true, billNo: returnDoc?.reference || '', party: returnDoc?.party || '', lines, total, note: returnDoc?.note || '' })
          setSearchTick(t=>t+1)
        }}
      />
