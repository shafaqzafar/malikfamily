...
export default function Lab_Login() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!uname || !password) { setError("Enter username and password"); return }

    try {
      // Lab-only login: try /lab/users/login; fallback to /lab/login if user collection auth is not present
      let res: any
      try {
        res = await labApi.loginUser(uname, password)
        localStorage.setItem('lab.auth.mode', 'lab')
      } catch (eUser: any) {
        const msg1 = String(eUser?.message || '')
        if (/404|Not Found|Cannot POST/i.test(msg1)) {
          res = await labApi.login(uname, password)
          localStorage.setItem('lab.auth.mode', 'lab')
        } else {
          throw eUser
        }
      ...
      const user = res?.user || { username: uname, role: 'Lab' }
      try {
        if (res?.token) {
          // Ensure Lab API calls authorize using module token; also keep legacy
          localStorage.setItem('lab.token', res.token)
              ...
              <form onSubmit={onSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e)=>setPassword(e.target.value)} className="w-full rounded-2xl border-2 border-white/10 bg-white/5 backdrop-blur-sm px-12 py-3.5 pr-14 text-white placeholder-white/40 outline-none transition-all focus:border-blue-400/50 focus:bg-white/10 focus:ring-4 focus:ring-blue-400/20" placeholder="Enter password" />
