...
export default function Pharmacy_SidebarPermissions() {
  ...
  const allowedPaths = (() => {
    ...
    if (allowedLabels.has('POS')) allowedLabels.add('Point of Sale')
    if (allowedLabels.has('Sales')) allowedLabels.add('Sales History')
    if (allowedLabels.has('SidebarPermissions')) allowedLabels.add('Sidebar Permissions')
    ...
    for (const item of pharmacySidebarNav) {
  ...
  const normalizePermissions = (perms: Permission[]) => {
    ...
    const allowedNav = allowedPaths ? pharmacySidebarNav.filter(i => allowedPaths.has(i.to)) : pharmacySidebarNav
  ...
  const loadPermissions = async () => {
    ...
    try {
      try {
        const r = await pharmacyApi.listSidebarRoles() as any
      ...
      const res = await pharmacyApi.listSidebarPermissions()
  ...
  const savePermissions = async () => {
    ...
    setSaving(true)
    try {
      await pharmacyApi.updateSidebarPermissions(selectedRole, {
