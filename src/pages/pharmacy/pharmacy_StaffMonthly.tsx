...
import Pharmacy_StaffReportDialog from '../../components/pharmacy/pharmacy_StaffReportDialog'
...
export default function Pharmacy_StaffMonthly(){
  ...
  useEffect(()=>{
    ...
    ;(async () => {
      try {
        const staffRes = await pharmacyApi.listStaff({ limit: 1000 })
        if (!mounted) return
  ...
  useEffect(()=>{
    ...
    ;(async () => {
      try {
        ...
        const res = await pharmacyApi.listAttendance({ from, to: toStr, staffId: selectedStaffId || undefined, limit: 1000 })
        if (!mounted) return
  ...
  const saveQuick = async (date: string, type:'in'|'out') => {
    ...
    await pharmacyApi.upsertAttendance(payload)
    // refresh
    const to = new Date(Number(month.slice(0,4)), Number(month.slice(5,7)), 0)
    const toStr = `${to.getFullYear()}-${String(to.getMonth()+1).padStart(2,'0')}-${String(to.getDate()).padStart(2,'0')}`
    const res = await pharmacyApi.listAttendance({ from: `${month}-01`, to: toStr, staffId: selectedStaffId })
    setAtt((res.items||[]).map((x:any)=>({ id: x._id || `${x.staffId}-${x.date}-${x.shiftId||''}`, staffId: x.staffId, date: x.date, shiftId: x.shiftId, status: x.status, clockIn: x.clockIn, clockOut: x.clockOut, notes: x.notes })))
  }
  ...
  const exportCsv = () => {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `pharmacy_monthly_${month}_${selectedStaff?.name||''}.csv`; a.click(); URL.revokeObjectURL(a.href)
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <Pharmacy_StaffReportDialog open={reportOpen} onClose={()=>setReportOpen(false)} staffList={staff as any} initialMonth={month} initialStaffId={selectedStaffId || undefined} />
    </div>
  )
}
