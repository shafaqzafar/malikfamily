...
export default function Pharmacy_UserManagement() {
  ...
  const addUser = async () => {
    ...
    try {
      const created = await pharmacyApi.createUser({ username: newUsername, role: newRole, password: newPassword }) as any
  ...
  const saveEdit = async () => {
    ...
    setSavingEdit(true)
    try {
      const updated = await pharmacyApi.updateUser(editing._id, { username: editing.username, role: editing.role }) as any
