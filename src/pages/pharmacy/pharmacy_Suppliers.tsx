...
export default function Pharmacy_Suppliers() {
  ...
  const startPayment = async (s: Supplier) => {
    ...
    try {
      const res = await pharmacyApi.listSupplierPurchases(s.id)
  ...
  const savePayment = async (s: Supplier) => {
    ...
    await pharmacyApi.recordSupplierPayment(s.id, { amount: amt, purchaseId: paymentDraft.purchaseId, method: paymentDraft.method, note: paymentDraft.note, date: paymentDraft.date })
            ...
            <div key={s.id} className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              {payOpen && (
                <div className="mt-3 grid gap-3 md:grid-cols-[1fr_1fr_160px_1fr_160px_auto_auto]">
                  <select value={paymentDraft.purchaseId || ''} onChange={e=>setPaymentDraft(p=>({ ...p, purchaseId: e.target.value || undefined }))} className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900">
