...
export default function Pharmacy_StaffSettings(){
  ...
  useEffect(()=>{
    ...
    ;(async () => {
      try {
        const res = await pharmacyApi.listShifts()
        if (!mounted) return
  ...
  const addShift = async () => {
    const created = await pharmacyApi.createShift({ name: `Shift ${shifts.length+1}`, start: '09:00', end: '17:00', absentCharges: 0, lateDeduction: 0, earlyOutDeduction: 0 })
    setShifts(s => [...s, {
      id: created._id,
      name: created.name,
      start: created.start,
  ...
  const save = async () => {
    ...
    try {
      ...
      await Promise.all(shifts.map(sh => pharmacyApi.updateShift(sh.id, {
        name: sh.name,
        start: sh.start,
        end: sh.end,
