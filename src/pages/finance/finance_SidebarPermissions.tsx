...
export default function Finance_SidebarPermissions() {
  ...
  const allowedPaths = (() => {
    ...
    for (const item of financeSidebarNav) {
  ...
  const normalizePermissions = (perms: Permission[]) => {
    ...
    const allowedNav = allowedPaths ? financeSidebarNav.filter(i => allowedPaths.has(i.to)) : financeSidebarNav
  ...
  const loadPermissions = async () => {
    ...
    try {
      try {
        const r = await financeApi.listSidebarRoles() as any
      ...
      const res = await financeApi.listSidebarPermissions()
  ...
  const savePermissions = async () => {
    ...
    setSaving(true)
    try {
      await financeApi.updateSidebarPermissions(selectedRole, {
