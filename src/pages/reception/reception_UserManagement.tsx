...
export default function Reception_UserManagement() {
  ...
  const [editing, setEditing] = useState<{ _id: string; username: string; role: User['role']; shiftId?: string; shiftRestricted?: boolean } | null>(null)
  ...
  async function load(){
    ...
    try {
      const [rolesRes, usersRes, shiftsRes] = await Promise.allSettled([
        (receptionApi as any).listSidebarRoles?.() as any,
        receptionApi.listUsers() as any,
        receptionApi.listShifts() as any,
      ])
      ...
      if (usersRes.status === 'fulfilled') {
        const arr = (usersRes.value?.items || []) as any[]
  ...
  const removeUser = async (_id: string) => {
    try {
      await receptionApi.deleteUser(_id)
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const updated = await receptionApi.updateUser(editing._id, payload) as any
      setUsers(prev => prev.map(u => (u._id === editing._id ? ({
        ...u,
        username: updated?.username ?? editing.username,
        role: updated?.role ?? editing.role,
        shiftId: updated?.shiftId ?? editing.shiftId,
        shiftRestricted: updated?.shiftRestricted ?? editing.shiftRestricted,
      }) : u)))
      setEditing(null)
    } catch {
    } finally {
  ...
  const createRole = async () => {
    ...
    try {
      await (receptionApi as any).createSidebarRole?.(role)
                ...
                <table className="min-w-full text-left text-sm">
                  <thead className="bg-slate-50/70 text-slate-700 dark:bg-slate-800/70 dark:text-slate-300">
                    <tr>
                      ...
                      <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wide dark:text-slate-400">Shift</th>
                      ...
                      <tr key={u._id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/70">
                        ...
                        <td className="px-5 py-3">
                          <div className="text-xs">
                            {assignedShift ? (
                              <div>
                                <div className="font-medium text-slate-700 dark:text-slate-300">{assignedShift.name}</div>
                                <div className="text-slate-500 dark:text-slate-400">{fmt12(assignedShift.start)}-{fmt12(assignedShift.end)}</div>
                                {u.shiftRestricted && <span className="mt-1 inline-flex items-center rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-medium text-amber-700 border border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800">Restricted</span>}
                              </div>
                            ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-white/40 bg-white/70 p-5 shadow-lg shadow-slate-200/50 backdrop-blur-md dark:border-slate-700/50 dark:bg-slate-900/70 dark:shadow-slate-950/30">
              ...
              <div className="mt-1 text-sm text-slate-600 dark:text-slate-400">
                ...
                <button
                  type="button"
                  onClick={() => navigate('/reception/sidebar-permissions')}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true">
        <div className="w-full max-w-md overflow-hidden rounded-2xl border border-white/20 bg-white/90 shadow-2xl backdrop-blur dark:border-slate-700 dark:bg-slate-900/90">
          ...
          <div className="space-y-3">
            ...
            <div className="px-5">
              <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-200">Shift</label>
              <select value={editing.shiftId || ''} onChange={e=>setEditing(prev => prev ? { ...prev, shiftId: e.target.value || undefined } : prev)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-200/60 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:ring-sky-900/40">
            ...
            <div className="px-5">
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={editing.shiftRestricted || false} onChange={e=>setEditing(prev => prev ? { ...prev, shiftRestricted: e.target.checked } : prev)} className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500" />
                <span className="text-sm text-slate-700 dark:text-slate-200">Restrict to shift hours</span>
              </label>
            </div>
          </div>
