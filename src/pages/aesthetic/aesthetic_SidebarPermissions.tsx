...
export default function Aesthetic_SidebarPermissions() {
  ...
  const normalizePermissions = (perms: Permission[]) => {
    ...
    const merged: Permission[] = aestheticSidebarNav.map((item, idx) => {
    ...
    const extra = (perms || []).filter(p => !aestheticSidebarNav.some(i => i.to === p.path))
  ...
  const loadPermissions = async () => {
    ...
    try {
      try {
        const r = await (aestheticApi as any).listSidebarRoles() as any
      ...
      const res = await (aestheticApi as any).listSidebarPermissions()
  ...
  const savePermissions = async () => {
    ...
    setSaving(true)
    try {
      await (aestheticApi as any).updateSidebarPermissions(selectedRole, {
