...
export default function Aesthetic_DoctorSchedules(){
  ...
  const [rowForm, setRowForm] = useState({ startTime: '09:00', endTime: '12:00', slotMinutes: '30', fee: '', followupFee: '', notes: '' })
  ...
  useEffect(()=>{ (async()=>{
    try{
      const docRes = await (aestheticApi as any).listDoctors() as any
  ...
  async function load(){
    setLoading(true)
    try{
      const res = await (aestheticApi as any).listDoctorSchedules({ doctorId, date: anchorDate }) as any
      setRows(res?.schedules || [])
    }catch{
      try{
        const resAll = await (aestheticApi as any).listDoctorSchedules({ date: anchorDate }) as any
  ...
  function beginEdit(s: Schedule){
    setEditingId(s._id)
    setRowForm({ startTime: s.startTime, endTime: s.endTime, slotMinutes: String(s.slotMinutes||15), fee: s.fee!=null?String(s.fee):'', followupFee: s.followupFee!=null?String(s.followupFee):'', notes: s.notes||'' })
  }

  async function saveEdit(id: string){
    try{
      await (aestheticApi as any).updateDoctorSchedule(id, {
  ...
  async function toggleSlots(s: Schedule){
    ...
    try{
      ...
      const usedTokens = new Set<number>((tk?.items || tk?.tokens || []).map((t:any)=> Number(t.slotNo||0)).filter(Boolean))
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
                      ...
                      <td colSpan={5} className="px-3 py-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {slotRows.map(r => {
                            ...
                            return (
                              <div key={r.slotNo} className={`rounded-md border px-2 py-1 text-xs ${base}`}> {r.slotNo} </div>
                            )
                          })}
                          {slotRows.length===0 && (
            ...
            {rows.length===0 && (
              <tr><td colSpan={5} className="px-4 py-6 text-center text-slate-500">No schedules</td></tr>
            )}
          </tbody>
        </table>
      </div>
