...
import Aesthetic_AddStaffDialog, { type PharmacyStaff } from '../../components/aesthetic/aesthetic_AddStaffDialog'
import Aesthetic_StaffEarningsDialog from '../../components/aesthetic/aesthetic_StaffEarningsDialog'
import { aestheticApi } from '../../utils/api'

type Shift = { id: string; name: string }

export default function Aesthetic_StaffManagement(){
    ...
    ;(async () => {
      try {
        const [staffRes, shiftRes] = await Promise.all([
          aestheticApi.listStaff({ page, limit }),
  ...
  const addStaff = async (s: PharmacyStaff) => {
    await aestheticApi.createStaff({
      name: s.name,
      position: s.position,
      phone: s.phone,
      joinDate: s.joinDate,
  ...
  const performDelete = async () => {
    ...
    try { await aestheticApi.deleteStaff(id); setReloadTick(t=>t+1); setNotice({ text: 'Staff deleted', kind: 'success' }) }
  ...
  const saveEdit = async (updated: PharmacyStaff) => {
    await aestheticApi.updateStaff(updated.id, {
      name: updated.name,
  ...
  return (
    <div className="space-y-4">
      ...
      <Aesthetic_AddStaffDialog open={addOpen} onClose={()=>setAddOpen(false)} onSave={addStaff} />
      <Aesthetic_AddStaffDialog open={editOpen} onClose={()=>setEditOpen(false)} onSave={saveEdit} initial={editing ?? undefined} title="Edit Staff" submitLabel="Save" />
      {earningsStaff && (
        <Aesthetic_StaffEarningsDialog
          open={earningsOpen}
