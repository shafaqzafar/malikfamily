...
import Pharmacy_PurchaseSlipDialog from '../../components/aesthetic/aesthetic_PurchaseSlipDialog'
...
export default function Pharmacy_PurchaseHistory() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res: any = await aestheticApi.listPurchases({ from: from || undefined, to: to || undefined, search: search || undefined, page, limit })
  ...
  const exportCsv = async () => {
    try {
      ...
      const first: any = await aestheticApi.listPurchases({ from: from || undefined, to: to || undefined, search: search || undefined, page: 1, limit: pageLimit })
      let items: any[] = (first.items || first || [])
      const tp = Number(first.totalPages || 1)
      if (tp > 1) {
        const pages = Array.from({ length: tp - 1 }, (_, i) => i + 2)
        const results = await Promise.all(pages.map(p => aestheticApi.listPurchases({ from: from || undefined, to: to || undefined, search: search || undefined, page: p, limit: pageLimit })))
      ...
      a.href = url
      a.download = `aesthetic_purchases_${y}-${m}-${day}.csv`
      ...
      try {
        ...
        const day = String(d.getDate()).padStart(2,'0')
        a.href = url
        a.download = `aesthetic_purchases_page_${page}_${y}-${m}-${day}.csv`
        document.body.appendChild(a)
        a.click()
        document.body.removeChild(a)
        URL.revokeObjectURL(url)
    ...
    <div className="space-y-4">
      ...
      <Aesthetic_ConfirmDialog
        ...
        onConfirm={async()=>{
          ...
          try {
            await aestheticApi.deletePurchase(toDelete.id)
