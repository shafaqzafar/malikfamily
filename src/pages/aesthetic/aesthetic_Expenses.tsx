...
export default function Pharmacy_Expenses() {
  ...
  const remove = async (id: string) => {
    await aestheticApi.deleteExpense(id)
    refresh()
  }
  ...
  const exportCSV = () => {
    ...
    const header = ['Date','Type','Note','Amount','User'].map(escape).join(',')
    const lines = items.map(e => [e.date, e.type, e.note, e.amount.toFixed(2), e.createdBy || ''].map(escape).join(',')).join('\n')
    const csv = header + '\n' + lines
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `aesthetic_expenses-${new Date().toISOString().slice(0,10)}.csv`
    a.click()
  ...
  const exportPDF = async () => {
    ...
    const cols = [
      ...
      { key: 'note', title: 'Note', width: 132 },
      { key: 'amount', title: 'Amount', width: 32 },
      { key: 'user', title: 'User', width: 44 },
    ] as const
    ...
    for (const e of items) {
      const data = [e.date, e.type, e.note, `Rs ${e.amount.toFixed(2)}`, e.createdBy || '']
    ...
    doc.save(`aesthetic_expenses-${new Date().toISOString().slice(0,10)}.pdf`)
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid gap-3 md:grid-cols-4 items-end">
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <input type="date" value={from} onChange={e=>setFrom(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="Start" />
              <input type="date" value={to} onChange={e=>setTo(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="End" />
            </div>
          </div>
          <div>
            <label className="mb-1 block text-sm text-slate-700">Expense Type</label>
            <select value={etype} onChange={e=>{ setEtype(e.target.value as any); setPage(1) }} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
          ...
          <div>
            ...
            <input type="number" value={minAmount} onChange={e=>setMinAmount(parseFloat(e.target.value || '0'))} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
          <div className="flex items-end gap-2">
            <button onClick={()=>{ setPage(1); refresh() }} className="btn">Apply</button>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <input value={search} onChange={e=>setSearch(e.target.value)} className="w-64 rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="Search expenses..." />
      ...
      <Pharmacy_AddExpenseDialog
        ...
        onSave={async (exp) => {
          await aestheticApi.createExpense(exp)
          setAddOpen(false)
          setPage(1)
          refresh()
        }}
