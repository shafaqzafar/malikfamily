...
export default function Patient_Login() {
  ...
  const [theme] = useState<'light' | 'dark'>(() => {
    try {
      return (localStorage.getItem('patient.theme') as 'light' | 'dark') || 'light'
    } catch {
      return 'light'
    }
  })
  ...
  const logoSrc = useMemo(() => `${(import.meta as any).env?.BASE_URL || '/'}hospital_icon.jpeg`, [])
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      await patientApi.login(uname, pwd)
      navigate('/patient')
    } catch (e2: any) {
      let msg = String(e2?.message || 'Invalid credentials')
      try {
      ...
      if (/failed to fetch/i.test(msg)) {
        msg = 'Backend not reachable. Start backend on http://127.0.0.1:4000'
      }
  ...
  const onSignupSubmit = async (e: React.FormEvent) => {
    ...
    if (!data.fullName || !data.phoneNumber || !data.dateOfBirth || !data.username || !data.password) {
    ...
    try {
      ...
      let msg = String(e2?.message || 'Registration failed')
      try {
        if (msg.trim().startsWith('{')) {
          const parsed = JSON.parse(msg)
          if (parsed?.message) msg = String(parsed.message)
      ...
      if (/failed to fetch/i.test(msg)) {
        msg = 'Backend not reachable. Start backend on http://127.0.0.1:4000'
      } else if (/409|exists|already/i.test(msg)) {
        msg = 'Username already exists'
      } else if (/400|validation|invalid/i.test(msg)) {
  ...
  return (
    <div className={theme === 'dark' ? 'patient-scope dark' : 'patient-scope'}>
      <div className="min-h-screen bg-gradient-to-br from-[#0a1e3c] via-[#0b2b55] to-slate-900 flex items-center justify-center p-4 overflow-hidden relative">
        ...
        <div className="card-3d relative w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-2xl backdrop-blur-2xl">
          ...
          <div className="relative">
            <div className="relative p-8 pb-6 text-center">
              ...
              <p className="text-sm text-white/60 font-medium">Patient Portal</p>
            </div>
            ...
            <div className="p-8 pt-4">
              <form onSubmit={onSubmit} className="space-y-5">
                ...
                <button disabled={busy} type="submit" className="group relative mt-2 w-full overflow-hidden rounded-2xl bg-gradient-to-r from-sky-500 via-blue-500 to-indigo-600 px-4 py-4 font-bold text-white shadow-2xl shadow-sky-500/30 transition-all hover:shadow-sky-500/50 hover:scale-[1.02] focus:outline-none focus:ring-4 focus:ring-sky-400/50 disabled:opacity-60 disabled:hover:scale-100">
        ...
        {signupOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-black/60" onClick={onCloseSignup} />
            <div className="relative w-full max-w-lg overflow-hidden rounded-3xl border border-white/10 bg-slate-900/70 shadow-2xl backdrop-blur-2xl">
              <div className="p-6 sm:p-8">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-black text-white">Create Patient Account</h2>
                    <p className="mt-1 text-sm text-white/60">Enter your details to register.</p>
                  </div>
                  <button disabled={signupBusy} onClick={onCloseSignup} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-white/70 hover:text-white hover:bg-white/10 disabled:opacity-50">
                    Close
                  </button>
                </div>
                ...
                <form onSubmit={onSignupSubmit} className="mt-6 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="sm:col-span-2">
                      <label className="mb-2 block text-sm font-bold text-white/90">Full Name</label>
                      <input value={signup.fullName} onChange={(e) => setSignup((s) => ({ ...s, fullName: e.target.value }))} className="w-full rounded-2xl border-2 border-white/10 bg-white/5 px-4 py-3 text-white placeholder-white/40 outline-none transition-all focus:border-sky-400/50 focus:bg-white/10 focus:ring-4 focus:ring-sky-400/20" placeholder="Full Name" />
                    </div>
                    <div>
                      <label className="mb-2 block text-sm font-bold text-white/90">Phone Number</label>
                      <input value={signup.phoneNumber} onChange={(e) => setSignup((s) => ({ ...s, phoneNumber: e.target.value }))} className="w-full rounded-2xl border-2 border-white/10 bg-white/5 px-4 py-3 text-white placeholder-white/40 outline-none transition-all focus:border-sky-400/50 focus:bg-white/10 focus:ring-4 focus:ring-sky-400/20" placeholder="Phone Number" />
                    </div>
                    <div>
                      <label className="mb-2 block text-sm font-bold text-white/90">Date of Birth</label>
                      <input type="date" value={signup.dateOfBirth} onChange={(e) => setSignup((s) => ({ ...s, dateOfBirth: e.target.value }))} className="w-full rounded-2xl border-2 border-white/10 bg-white/5 px-4 py-3 text-white placeholder-white/40 outline-none transition-all focus:border-sky-400/50 focus:bg-white/10 focus:ring-4 focus:ring-sky-400/20" />
                    </div>
                    <div>
                      <label className="mb-2 block text-sm font-bold text-white/90">Username</label>
                      <input value={signup.username} onChange={(e) => setSignup((s) => ({ ...s, username: e.target.value }))} className="w-full rounded-2xl border-2 border-white/10 bg-white/5 px-4 py-3 text-white placeholder-white/40 outline-none transition-all focus:border-sky-400/50 focus:bg-white/10 focus:ring-4 focus:ring-sky-400/20" placeholder="Username" autoComplete="username" />
                    </div>
                    <div>
                      <label className="mb-2 block text-sm font-bold text-white/90">Password</label>
                      <input type="password" value={signup.password} onChange={(e) => setSignup((s) => ({ ...s, password: e.target.value }))} className="w-full rounded-2xl border-2 border-white/10 bg-white/5 px-4 py-3 text-white placeholder-white/40 outline-none transition-all focus:border-blue-400/50 focus:bg-white/10 focus:ring-4 focus:ring-blue-400/20" placeholder="Password" autoComplete="new-password" />
                    </div>
                  </div>

                  {signupError && (
                    <div className="rounded-2xl border-2 border-rose-400/30 bg-rose-500/10 backdrop-blur-sm px-4 py-3 text-sm text-rose-200 font-medium">{signupError}</div>
                  )}

                  <button disabled={signupBusy} type="submit" className="w-full rounded-2xl bg-gradient-to-r from-emerald-500 via-sky-500 to-indigo-600 px-4 py-3.5 font-bold text-white shadow-2xl shadow-emerald-500/20 transition-all hover:scale-[1.01] focus:outline-none focus:ring-4 focus:ring-emerald-400/40 disabled:opacity-60 disabled:hover:scale-100">
                    Register
                  </button>
                </form>
              </div>
