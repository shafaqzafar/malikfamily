...
import { hospitalSidebarNav } from '../../components/hospital/hospital_Sidebar'
...
export default function Hospital_SidebarPermissions() {
  ...
  const allowedPaths = (() => {
    ...
    for (const item of hospitalSidebarNav) {
  ...
  const normalizePermissions = (perms: Permission[]) => {
    ...
    const allowedNav = allowedPaths ? hospitalSidebarNav.filter(i => allowedPaths.has(i.to)) : hospitalSidebarNav
  ...
  const loadPermissions = async () => {
    ...
    try {
      try {
        const r = await hospitalApi.listSidebarRoles() as any
      ...
      const res = await hospitalApi.listSidebarPermissions()
  ...
  const confirmDeleteRoleAction = async () => {
    ...
    if (!role) return
    try {
      await hospitalApi.deleteSidebarRole(role)
  ...
  const savePermissions = async () => {
    ...
    setSaving(true)
    try {
      await hospitalApi.updateSidebarPermissions(selectedRole, {
  ...
  const confirmResetAction = async () => {
    ...
    setSaving(true)
    try {
      await hospitalApi.resetSidebarPermissions(role)
      await loadPermissions()
        ...
        <div className="mb-6">
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-[1fr_auto]">
            <input
              ...
              placeholder="Create new role (e.g. ward-admin)"
            />
            <button
              type="button"
              onClick={createRole}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                {!['admin','staff'].includes(String(selectedRole || '').toLowerCase()) && (
                  <button
                    type="button"
