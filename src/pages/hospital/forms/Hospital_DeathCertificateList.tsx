...
export default function Hospital_DeathCertificateList(){
  ...
  async function load(){
    setLoading(true)
    try {
      const res: any = await hospitalApi.listIpdDeathCertificates({ q, page, limit, encounterType: encounterType || undefined }).catch(()=>null)
      if (res && Array.isArray(res.results)){
        setRows(res.results)
        setTotal(res.total||res.results.length||0)
        return
      }
      // Fallback: list discharged encounters and include those with a certificate
      ...
      const mapped = await Promise.all(admissions.map(async (e: any)=>{
        try {
          const dc: any = await hospitalApi.getIpdDeathCertificate(String(e._id)).catch(()=>null)
          if (dc?.certificate){
            return {
              _id: dc.certificate._id,
              encounterId: String(e._id),
              createdAt: dc.certificate.createdAt || e.startAt,
  ...
  async function onPrint(formId: string){
    try {
      const html = await coreApi(`/hospital/ipd/admissions/${encodeURIComponent(formId)}/death-certificate/print`) as any
      const api: any = (window as any).electronAPI
      if (api && typeof api.printPreviewHtml === 'function') { await api.printPreviewHtml(String(html), {}); return }
      const w = window.open('', '_blank'); if (!w) return; w.document.write(String(html)); w.document.close(); w.focus()
    } catch {}
  }
  ...
  async function confirmDelete(){
    ...
    try { await hospitalApi.deleteIpdDeathCertificate(confirmDeleteId) } catch {}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-lg font-semibold text-slate-800">Death Certificates</div>
              ...
              <tr key={r._id} className="border-t">
                ...
                <td className="px-3 py-2">
                  <div className="flex gap-2">
                    <button className="btn-outline-navy text-xs" onClick={()=> navigate(`/hospital/ipd/admissions/${encodeURIComponent(r.encounterId)}/forms/death-certificate`)}>Edit</button>
