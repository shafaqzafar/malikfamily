...
export default function Hospital_Appointments(){
      ...
      for (const s of rows){
        try {
          const [ap, tk]: any = await Promise.all([
            hospitalApi.listAppointments({ scheduleId: String(s._id), includePatientPortal: '1' as any }),
  ...
  async function loadApptTable(){
    ...
    setTblLoading(true)
    try{
      // Include patient-portal appointments in the Hospital Appointments page by passing includePatientPortal=1
      // This allows hospital staff to see and manage appointments booked via the patient portal
      const params: any = { date: dateIso, includePatientPortal: '1' }
  ...
  async function convert(id: string){
    ...
    try{
      ...
      const errorMsg = e?.response?.data?.error || e?.message || 'Failed to convert to token'
      console.error('Convert to token error:', e?.response?.data || e)
      setError(errorMsg)
    }
  }
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-slate-50 text-slate-600">
            <tr>
              ...
              <th className="px-3 py-2 text-left">Consultant Fee</th>
          ...
          <tbody>
            {apptRows.map((appt:any) => {
              ...
              return (
                <tr key={String(appt._id)} className="border-b border-slate-100">
                  ...
                  <td className="px-3 py-2">{appt.mrn || '-'}</td>
                  <td className="px-3 py-2">{appt.patientName || '—'}</td>
                  <td className="px-3 py-2">{appt.age || '-'}</td>
                  <td className="px-3 py-2">{appt.phoneNormalized || '-'}</td>
                  <td className="px-3 py-2">{appt.status}</td>
                  <td className="px-3 py-2">{appt.fee != null ? Number(appt.fee).toLocaleString() : '-'}</td>
                  <td className="px-3 py-2">
                    {appt.patientUpload?.dataBase64 ? (
                      <button
                        onClick={() => {
                          // Build proper data URL with mime type prefix if not present
                          let dataUrl = appt.patientUpload.dataBase64
                          if (!dataUrl.startsWith('data:')) {
                            const mime = appt.patientUpload?.mimeType || 'image/png'
                            dataUrl = `data:${mime};base64,${dataUrl}`
                          }
                          setViewImage({ url: dataUrl, title: appt.patientUpload?.fileName || 'Screenshot' })
                        }}
            ...
            {!tblLoading && apptRows.length===0 && (
              <tr><td colSpan={9} className="px-4 py-6 text-center text-slate-500">No appointments</td></tr>
            )}
          </tbody>
        </table>
