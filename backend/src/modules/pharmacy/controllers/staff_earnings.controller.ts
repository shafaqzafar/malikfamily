...
import { PharmacyStaffEarning } from '../models/StaffEarning'
...
export async function list(req: Request, res: Response){
  ...
  const total = await PharmacyStaffEarning.countDocuments(filter)
  const items = await PharmacyStaffEarning.find(filter).sort({ date: -1, createdAt: -1 }).skip(skip).limit(effectiveLimit).lean()
...
export async function create(req: Request, res: Response){
  ...
  const row: any = await PharmacyStaffEarning.create(toInsert)
  try {
...
export async function update(req: Request, res: Response){
  ...
  const row: any = await PharmacyStaffEarning.findByIdAndUpdate(req.params.id, toUpdate, { new: true })
...
export async function remove(req: Request, res: Response){
  const before: any = await PharmacyStaffEarning.findById(req.params.id).lean()
  await PharmacyStaffEarning.findByIdAndDelete(req.params.id)
  try {
    const actor = (req as any).user?.name || (req as any).user?.email || 'system'
    await AuditLog.create({
      actor,
