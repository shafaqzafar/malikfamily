...
export async function list(req: Request, res: Response) {
  ...
  const total = await LabSupplier.countDocuments(filter)
  const skip = (page - 1) * limit
  const items: any[] = await LabSupplier.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).lean()
  ...
  if (ids.length){
    ...
    purchaseAgg = await LabPurchase.aggregate([
      { $match: { $or: [
        { supplierId: { $in: ids } },
    ...
    paymentAgg = await LabSupplierPayment.aggregate([
...
export async function create(req: Request, res: Response) {
  ...
  const s = await LabSupplier.create(data)
  try {
...
export async function update(req: Request, res: Response) {
  ...
  const s = await LabSupplier.findByIdAndUpdate(id, data, { new: true })
...
export async function remove(req: Request, res: Response) {
  const id = req.params.id
  const before: any = await LabSupplier.findById(id).lean()
  const s = await LabSupplier.findByIdAndDelete(id)
  if (!s) throw new ApiError(404, 'Supplier not found')
  try {
    const actor = (req as any).user?.name || (req as any).user?.email || 'system'
    await LabAuditLog.create({
...
export async function recordPayment(req: Request, res: Response) {
  ...
  const s = await LabSupplier.findById(id)
  if (!s) throw new ApiError(404, 'Supplier not found')
  await LabSupplierPayment.create({ supplierId: id, purchaseId, amount, method, note, date })
...
export async function purchases(req: Request, res: Response) {
  ...
  const sup = await LabSupplier.findById(id).lean()
  const name = String((sup as any)?.name || '')
  const nameRx = name ? new RegExp(`^${rxEscape(name)}$`, 'i') : null
  const purchases = await LabPurchase.find(nameRx ? { $or: [ { supplierId: id }, { supplierName: nameRx } ] } : { supplierId: id }).sort({ date: -1, invoice: -1 }).lean()
  const payAgg = await LabSupplierPayment.aggregate([
