...
export async function list(req: Request, res: Response){
  ...
  const [items, total] = await Promise.all([
    LabInventoryItem.find(filter).sort({ name: 1 }).skip(skip).limit(limit).lean(),
    LabInventoryItem.countDocuments(filter),
  ])
...
export async function summary(req: Request, res: Response){
  ...
  const allItems = await LabInventoryItem.find(filter).lean()
  const now = new Date()
...
export async function remove(req: Request, res: Response){
  ...
  await LabInventoryItem.findOneAndDelete({ key: norm })
  res.json({ ok: true })
}
...
export async function update(req: Request, res: Response){
  ...
  const doc = await (LabInventoryItem as any).findOne({ key: norm })
