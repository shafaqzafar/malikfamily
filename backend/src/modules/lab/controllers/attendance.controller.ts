...
import { LabAttendance } from '../models/Attendance'
...
export async function list(req: Request, res: Response) {
  ...
  const total = await LabAttendance.countDocuments(filter)
  const items = await LabAttendance.find(filter)
...
export async function upsert(req: Request, res: Response) {
  ...
  const doc = await LabAttendance.findOneAndUpdate(key, { $set: data }, { new: true, upsert: true })
  try {
