...
export async function list(req: Request, res: Response){
  ...
  const total = await LabStaffEarning.countDocuments(filter)
  const items = await LabStaffEarning.find(filter).sort({ date: -1, createdAt: -1 }).skip(skip).limit(effectiveLimit).lean()
...
export async function create(req: Request, res: Response){
  ...
  const row: any = await LabStaffEarning.create(toInsert)
  try {
...
export async function update(req: Request, res: Response){
  ...
  const row: any = await LabStaffEarning.findByIdAndUpdate(req.params.id, toUpdate, { new: true })
...
export async function remove(req: Request, res: Response){
  const before: any = await LabStaffEarning.findById(req.params.id).lean()
  await LabStaffEarning.findByIdAndDelete(req.params.id)
  try {
    const actor = (req as any).user?.name || (req as any).user?.email || 'system'
    await LabAuditLog.create({
      actor,
