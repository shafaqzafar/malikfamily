...
export async function create(req: Request, res: Response){
  try{
    ...
    const doc = await AestheticDoctor.findById(data.doctorId).lean()
    ...
    const row = await AestheticDoctorSchedule.create({ ...data })
...
export async function list(req: Request, res: Response){
  try{
    ...
    const rows = await AestheticDoctorSchedule.find(crit).sort({ dateIso: 1, startTime: 1 }).lean()
...
export async function update(req: Request, res: Response){
  try{
    ...
    const row = await AestheticDoctorSchedule.findByIdAndUpdate(id, { $set: data }, { new: true })
...
export async function remove(req: Request, res: Response){
  try{
    const id = String(req.params.id)
    const row = await AestheticDoctorSchedule.findByIdAndDelete(id)
    if (!row) return res.status(404).json({ error: 'Schedule not found' })
    res.json({ ok: true })
  }catch{ res.status(500).json({ error: 'Internal Server Error' }) }
}
...
export async function applyWeeklyPattern(req: Request, res: Response){
  try{
    ...
    const doctor = await AestheticDoctor.findById(payload.doctorId).lean()
    ...
    const result = await AestheticDoctorSchedule.bulkWrite(ops, { ordered: false })
