...
export async function list(_req: Request, res: Response) {
  const items = await ReceptionShift.find().sort({ createdAt: 1 }).lean()
  res.json({ items })
}

export async function create(req: Request, res: Response) {
  const data = shiftCreateSchema.parse(req.body)
  const s = await ReceptionShift.create(data)
...
export async function update(req: Request, res: Response) {
  ...
  const s = await ReceptionShift.findByIdAndUpdate(id, data, { new: true })
...
export async function remove(req: Request, res: Response) {
  const id = req.params.id
  await ReceptionShift.findByIdAndDelete(id)
  res.json({ ok: true })
}
